...
local function create_throbber(buf, rowstart, colstart, rowend, colend, replacementCallback)
	...
	local mark1 = vim.api.nvim_buf_set_extmark(buf, namespace, rowstart-1,  0, {virt_lines_above=true,right_gravity=false,virt_text={{''}},virt_lines={{{chars[iteration] .. ' Implementing'}}}})
	local mark2 =	vim.api.nvim_buf_set_extmark(buf, namespace, rowend  -0, -0, {virt_lines_above=true,right_gravity=false,virt_text={{''}},virt_lines={{{chars[iteration] .. ' Implementing'}}}})
